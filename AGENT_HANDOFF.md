...
- **Simplified the build.** `update-pcatt-app.mjs` now targets only `index.html`, uses the extractor's `type` field, and skips presenter-less blocks when building the speaker list. Panelists are now included as speakers (36 total).
